...
app.post("/api/apartments", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Apartment added successfully!", data: result });
  } catch (error) {
...
app.put("/api/apartments/:id", async (req, res) => {
  const { old_apartmentId, apartmentId, bedrooms, bathrooms, logia, area } =
    req.body;

  const query =
    "UPDATE `apartment` SET `apartment_id`=?, `bedrooms`=?, `bathrooms`=?, `loggias`=?, `area`=? WHERE `apartment_id`=?"; // Call the stored procedure
...
app.put("/api/residents/:id", async (req, res) => {
  ...
  const query =
    "UPDATE `resident` SET `resident_id`=?, `full_name`=?, `apartment_id`=?, `birth_date`=?, `phone_number`=?, `email`=? WHERE `resident_id`=?"; // Call the stored procedure
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Residents updated successfully!", data: result });
  } catch (error) {
    console.error("SQL Error:", error);
    res.status(500).json({ message: "Database error: " + error.sqlMessage });
  }
...
app.put("/api/vehicles/:id", async (req, res) => {
  ...
  const query =
    "UPDATE `vehicle` SET `license_plate`=?, `apartment_id`=?, `type`=?, `status`=? WHERE `license_plate`=?"; // Call the stored procedure
...
app.post("/api/vehicle-out/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Vehicle out successfully!", data: result });
  } catch (error) {
    console.error("SQL Error:", error);
    res.status(500).json({ message: "Database error: " + error.sqlMessage });
  }
...
app.put("/api/service-fee/:id", async (req, res) => {
  ...
  const query =
    "UPDATE `service_fee` SET `id`=?, `apartment_id`=?, `time_period`=?, `electricity_usage`=?, `water_usage`=?, `cleaning_fee`=?, `parking_fee`=?, `status`=? WHERE `id`=?"; // Call the stored procedure
